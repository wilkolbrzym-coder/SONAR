...
pub struct EngineConfig {
    ...
    pub rules: GameRules,
    /// Enable the exact-CENSUS endgame solver (0.3): submarine-perfect
    /// play when 1–2 enemy ships remain. Defaults to `true`.
    #[serde(default = "default_use_endgame")]
    pub use_endgame: bool,
}

fn default_use_endgame() -> bool {
    true
}
...
impl Engine {
    ...
    pub fn new(config: EngineConfig) -> Self {
        ...
        Self {
            strategy: Box::new(
                HybridTargeting::new()
                    .with_soft_target(config.hypothesis_soft_target)
                    .with_endgame(config.use_endgame),
            ),
            our_board: Board::new(),
    ...
    pub fn apply_config(&mut self) {
        ...
        self.strategy = Box::new(
            HybridTargeting::new()
                .with_soft_target(self.config.hypothesis_soft_target)
                .with_endgame(self.config.use_endgame),
        );
    }
