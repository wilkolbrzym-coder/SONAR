...
//! a learning database handle, and a configurable move deadline.
//!
//! ## Design principles
//!
...
//!   matrix — for debugging and UIs.
//! - **Learning-aware.** When `use_learning` is `true`, the engine records
//!   every finished game into a JSON database and biases future placement
//!   and targeting decisions accordingly.
...
pub struct EngineConfig {
    ...
    /// Enable micro-learning (record games + bias future decisions).
...
impl Engine {
    ...
    /// Number of surviving hypotheses in the Bayesian filter (0 if the
    /// strategy is not a `HybridTargeting`).
    pub fn hypothesis_count(&self) -> usize {
        // We can't downcast `dyn TargetingStrategy` without `Any`. As a
        // pragmatic workaround, we keep `moves_fired` and rely on the
        // strategy's internal state. This method returns 0 for non-hybrid
        // strategies, which is fine because they don't use hypotheses.
        0
    }

    /// Latest 100-cell probability matrix from the hypothesis filter.
    /// Returns zeroes if no hypotheses are active.
    ...
    /// Record the current game into the learning database. Called when
    /// the game ends.
...
mod tests {
    ...
    fn test_engine_plays_a_game() {
        ...
        // Use a short deadline (10ms) to keep the test fast.
