//! A 128-bit BitBoard for the 10×10 board.
//!
//! Convention: cell (r, c) with r, c in 0..=9; bit index = r * 10 + c
//! (0..=99). The mask of all 100 cells is `MASK_100`.
//!
//! All operations are inline and compile to 2–3 SIMD instructions on
//! x86_64.

/// Bit representation of the 10×10 board (lower 100 bits of a `u128`).
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash, Default)]
pub struct BitBoard(pub u128);

/// Mask of all 100 board cells.
pub const MASK_100: u128 = (1u128 << 100) - 1;

/// Column 0 mask (left edge) — bits 0, 10, 20, ..., 90.
...
/// Column 9 mask (right edge) — bits 9, 19, 29, ..., 99.
...
/// Pre-computed 8-neighbour masks for all 100 cells (lazily filled
/// once via `OnceLock`).
pub static NEIGHBORS8_CACHE: std::sync::OnceLock<[u128; 100]> = std::sync::OnceLock::new();

/// Fetch the cached 8-neighbour mask of cell (r, c).
#[inline(always)]
...
/// Pre-computed 4-neighbour (von Neumann) masks for all 100 cells.
...
impl BitBoard {
    /// A row mask in the bitboard (10 bits).
    #[inline(always)]
...
impl BitBoard {
    ...
    /// Number of set bits (popcount — a single CPU instruction).
    #[inline(always)]
    ...
    /// Does cell (r, c) neighbour (8-directionally) any set bit?
    #[inline]
    ...
    /// The 8-neighbour mask of cell (r, c), clipped to the board.
    #[inline]
    ...
    /// The 4-neighbour mask (up/down/left/right).
    #[inline]
    ...
    /// Shift left by 1 (on the board: r, c-1) — with edge masking.
    #[inline(always)]
    pub fn shift_left(self) -> BitBoard {
        // Shift first, then clear the right column to kill wraparound.
        let shifted = self.0 >> 1;
        BitBoard(shifted & !COL_RIGHT)
    }

    /// Shift right by 1 (r, c+1).
    #[inline(always)]
    ...
    /// Dilate: expand by 1 cell in all 8 directions (Moore neighbourhood).
    #[inline]
    pub fn dilate8(self) -> BitBoard {
        // 4 von Neumann neighbours (up/down/left/right).
        let mut r = self.0;
        r |= (self.0 >> 1) & !COL_RIGHT;
        r |= (self.0 << 1) & !COL_LEFT & MASK_100;
        r |= self.0 >> 10;
        r |= (self.0 << 10) & MASK_100;
        // 4 diagonals (corners) — diagonal dilations.
        // LEFT-UP: shift_left + shift_up
        r |= (self.0 >> 11) & !COL_RIGHT;
        // RIGHT-UP: shift_right + shift_up
        r |= (self.0 >> 9) & !COL_LEFT;
        // LEFT-DOWN: shift_left + shift_down
        r |= (self.0 << 9) & !COL_RIGHT & MASK_100;
        // RIGHT-DOWN: shift_right + shift_down
        r |= (self.0 << 11) & !COL_LEFT & MASK_100;
        BitBoard(r & MASK_100)
    }
}

/// Iterator over set bits — very fast (trailing_zeros-based).
pub struct BitIter {
    bits: u128,
}
...
mod tests {
    ...
    fn test_edge_wrap_prevention() {
        // A bit in column 0 must not wrap after shift_right.
        let mut b = BitBoard::new();
        b.set(0, 0);
        ...
        // A bit in column 9 must not wrap after shift_left.
    ...
    fn test_dilate() {
        ...
        b.set(5, 5);
        let d = b.dilate8();
        assert_eq!(d.popcount(), 9); // the centre + 8 neighbours
    }
    ...
    fn test_column_masks() {
        // COL_LEFT must have 10 bits (one per row, column 0).
    ...
    fn test_dilate_corner_ship() {
        // A ship in column 9, rows 4–7 (vertical, length 4).
        let mut b = BitBoard::new();
        b.set(4, 9);
        b.set(5, 9);
        b.set(6, 9);
