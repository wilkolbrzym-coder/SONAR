...
#
# 0.5: the build enables the wasm128 SIMD backend of sonar-simd
# (`-C target-feature=+simd128`) — the generalised engine's tiered
# bitboards then dispatch their hot loops to v128 code.
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo ">>> Building sonar-wasm (release, simd128)"
RUSTFLAGS="-C target-feature=+simd128" cargo build --release --target "$TARGET" -p sonar-wasm
