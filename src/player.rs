//! Trait gracza i implementacje: Bot, Human (CLI), SuggestBot.
...
pub trait Player: Send {
    ...
    /// Plansza gracza (z jego statkami)
    fn board(&self) -> &Board;
    fn board_mut(&mut self) -> &mut Board;

    /// Wybierz ruch przeciwko wrogowi (z twardym deadlinem)
    fn choose_move(&mut self, deadline: Deadline) -> (usize, usize);

    /// Zaobserwuj wynik swojego strzału
    fn observe_result(&mut self, r: usize, c: usize, result: ShotResult);

    /// Zaobserwuj strzał wroga w twoją planszę (już wykonany - dla UI/debug)
    fn observe_incoming(&mut self, _r: usize, _c: usize, _result: ShotResult) {}

    /// Czy przegrał (wszystkie statki zatopione)
    fn is_defeated(&self) -> bool {
        self.board().all_sunk()
    }

    /// Reset gracza do nowej gry
    fn reset(&mut self);
}

/// Bot - hybrydowy PDF + Hipotezy
pub struct BotPlayer {
    ...
    /// Lista wykonanych strzałów z wynikiem (do micro-learningu)
    pub my_shots: Vec<(u8, bool)>,
    /// Włączenie micro-learningu
    pub use_learning: bool,
    /// Domyślny deadline na ruch
...
impl BotPlayer {
    ...
    /// Ustaw deadline na ruch
    pub fn with_deadline(mut self, deadline: Deadline) -> Self {
        self.move_deadline = deadline;
        self
    }

    /// Wyłącz learning (do benchmarków)
    pub fn without_learning(mut self) -> Self {
        self.use_learning = false;
        self
    }

    /// Zaaplikuj learning bias z globalnej DB
    pub fn apply_learning(&mut self) {
        if !self.use_learning { return; }
        let db = learning::global();
        if let Some(db) = db.as_ref() {
            if !db.is_empty() {
                let bias = db.targeting_bias(0.92);
                // Wstrzyknij bias do PDF w HybridTargeting
                // To wymaga downcastu - ale HybridTargeting ma publiczne pole pdf
                // Używamy unsafe trick: Box<dyn> zamieniamy na konkret
                // W praktyce - prościej: zachowajmy referencję do PdfTargeting
                // Tu: cast przez trait object method - dodaj metodę w trait? Nie.
                // Robimy inaczej: przechowujemy learning_bias i podajemy przy choose
                let _ = bias; // TODO: pass via context
            }
        }
    }
    ...
    /// Zapisz grę do micro-learning DB
...
impl Player for BotPlayer {
    ...
    fn choose_move(&mut self, deadline: Deadline) -> (usize, usize) {
        // If the caller passes a real deadline, use it. Otherwise use the
        // engine's default move deadline. But if the engine's default is
        // also None (e.g. in benchmarks), pass None through so the
        // strategy returns immediately.
...
/// Słabszy bot - tylko PDF (do benchmarków porównawczych)
pub struct PdfBot {
    pub name: String,
    pub board: Board,
    pub view: EnemyView,
...
/// Człowiek - wejście z stdin (lub innej implementacji)
pub struct HumanPlayer {
    pub name: String,
    pub board: Board,
    pub view: EnemyView,
    /// Fn pobierająca ruch od gracza: (r, c)
